...
export function ClientPortalView({ client, appointments, tokenEmail }: {
  client: Client
    ...
    <div className="min-h-screen bg-gradient-to-br from-tramit-blue-light via-white to-slate-50 dark:from-slate-900 dark:via-slate-800 dark:to-slate-900">
      ...
      <div className="max-w-2xl mx-auto px-4 py-8 space-y-6">
        ...
        <div className="text-center space-y-2">
          ...
          <h1 className="text-2xl font-bold">Benvingut/da, {client.name.split(' ')[0]}</h1>
          ...
          <CardContent className="space-y-2">
            ...
            {client.email && (
              <div className="flex items-center gap-2 text-sm">
                ...
                <a href={`mailto:${client.email}`} className="text-tramit-blue hover:underline">{client.email}</a>
              </div>
            )}
            {client.phone && (
              <div className="flex items-center gap-2 text-sm">
                <Phone className="h-4 w-4 text-muted-foreground shrink-0" />
                <a href={`tel:${client.phone}`} className="hover:text-tramit-blue">{client.phone}</a>
              </div>
            )}
            {client.nif_cif && (
            ...
            {appointments.length === 0 ? (
              ...
              <div className="space-y-3">
                {appointments.map(apt => {
                  ...
                  return (
                    <div key={apt.id} className="p-4 rounded-xl bg-tramit-blue-light dark:bg-blue-900/20 border border-tramit-blue/20">
                      <div className="flex items-start gap-3">
                        ...
                        <div className="flex-1">
                          <p className="font-semibold text-sm">{TOPIC_LABELS[apt.topic] || apt.topic}</p>
        ...
        <Card>
          <CardContent className="pt-4 pb-4 text-center space-y-3">
            <p className="text-sm font-medium">Necessites ajuda o vols concertar una cita?</p>
