...
import { CheckCircle, XCircle, Clock, Umbrella, ChevronDown, ChevronUp, Pencil, X, Save } from 'lucide-react'
...
export function VacancesAdminClient({ requests, balances }: { requests: Request[]; balances: Balance[] }) {
    ...
    if (action === 'approved') {
      ...
      if (req) {
        ...
        if (balance) {
          ...
          }).eq('user_id', req.user_id).eq('year', new Date().getFullYear())
        }
      }
    }
  ...
  async function saveBalance(bal: Balance) {
    ...
    await supabase.from('vacation_balances').update({
      total_days: edit.total_days,
      used_days: edit.used_days,
    }).eq('id', bal.id)
  ...
  return (
    <div className="space-y-6 max-w-4xl">
      ...
      <div className="flex gap-1 bg-muted p-1 rounded-lg w-fit">
        <button onClick={() => setTab('sol·licituds')}
          ...
          Sol·licituds
          {pendingCount > 0 && <span className="ml-1 bg-amber-500 text-white text-xs rounded-full px-1.5 py-0.5">{pendingCount}</span>}
        </button>
        <button onClick={() => setTab('saldos')}
          ...
          <div className="flex gap-2 flex-wrap">
            {(['all', 'pending', 'approved', 'rejected'] as const).map(f => (
              <button key={f} onClick={() => setFilter(f)}
                className={`px-3 py-1.5 rounded-full text-xs font-medium transition-all ${filter === f ? 'bg-tramit-blue text-white' : 'bg-muted text-muted-foreground hover:text-foreground'}`}>
                  ...
                  <CardContent className="pt-4 pb-4">
                    <div className="flex items-start justify-between gap-4">
                      <div className="flex-1">
                        ...
                        {req.notes && <p className="text-xs text-muted-foreground mt-1 italic">&ldquo;{req.notes}&rdquo;</p>}
                        {req.admin_note && <p className="text-xs text-muted-foreground mt-1"><span className="font-medium">Nota admin:</span> {req.admin_note}</p>}
                      </div>
                      {req.status === 'pending' && (
                        <button onClick={() => setExpanded(expanded === req.id ? null : req.id)} className="text-muted-foreground hover:text-foreground transition-colors">
                    ...
                    {req.status === 'pending' && expanded === req.id && (
                      <div className="mt-4 pt-4 border-t space-y-3">
                        <div className="space-y-1.5">
                          ...
                          <input type="text" placeholder="Escriu una nota..."
                        ...
                        <div className="flex gap-2">
                          <Button size="sm" variant="tramit" disabled={loading === req.id}
                            onClick={() => handleAction(req.id, 'approved')}
                            className="flex items-center gap-1.5">
                            <CheckCircle className="h-3.5 w-3.5" />Aprovar
                          </Button>
                          <Button size="sm" variant="outline" disabled={loading === req.id}
                            onClick={() => handleAction(req.id, 'rejected')}
                            className="flex items-center gap-1.5 text-red-600 hover:text-red-700 border-red-200 hover:bg-red-50">
                            <XCircle className="h-3.5 w-3.5" />Rebutjar
                          </Button>
                        </div>
                      </div>
                    )}
      ...
      {tab === 'saldos' && (
        <div className="space-y-3">
          <p className="text-sm text-muted-foreground">
            Clica el botó d&apos;edició per modificar manualment els dies d&apos;un treballador. Tots els canvis queden registrats a l&apos;auditoria.
          </p>
          ...
          {balances.map(bal => {
            ...
            return (
              <Card key={bal.id} className={isEditing ? 'border-tramit-blue/50 dark:border-tramit-blue/50' : ''}>
                <CardContent className="pt-4 pb-4">
                  {!isEditing ? (
                    <div className="space-y-2">
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-3">
                          ...
                          <div className="flex items-center gap-2 text-xs text-muted-foreground">
                            ...
                            {bal.pending_days > 0 && (
                              <span className="text-amber-600 dark:text-amber-400">({bal.pending_days} pendents)</span>
                            )}
                          </div>
                        </div>
                        <button onClick={() => startEditBalance(bal)}
                      ...
                      <div className="h-2 bg-muted rounded-full overflow-hidden">
                        <div className="h-full rounded-full bg-tramit-blue transition-all" style={{ width: `${pct}%` }} />
                      </div>
                      {saveSuccess === bal.id && (
                    ...
                    <div className="space-y-4">
                      <div className="flex items-center justify-between">
                        ...
                        <button onClick={() => setEditingBalance(null)} className="text-muted-foreground hover:text-foreground">
                          <X className="h-4 w-4" />
                        </button>
                      </div>
                      ...
                      {edit && (
                        <div className="rounded-lg bg-tramit-blue-light dark:bg-blue-900/20 px-4 py-2.5">
                          <p className="text-sm text-tramit-blue dark:text-blue-300">
                            Dies restants: <span className="font-bold text-base">{edit.total_days - edit.used_days}</span>
                          </p>
                        </div>
                      )}

                      <div className="space-y-1.5">
                        <label className="text-xs font-medium">Motiu de l&apos;ajust <span className="text-muted-foreground">(quedarà a l&apos;auditoria)</span></label>
                        <Input
                          placeholder="Ex: Correccció per acord especial, error de càlcul..."
                          value={edit?.note || ''}
                          onChange={e => setBalanceEdits(prev => ({
                            ...prev,
                      ...
                      <div className="flex gap-2">
                        <Button size="sm" variant="tramit" disabled={loading === bal.id}
